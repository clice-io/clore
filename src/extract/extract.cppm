...
auto resolve_path_under_directory(const std::string &path,
  ...
  if (path.empty()) {
    return std::unexpected(ExtractError{
        .message = "compilation database entry has empty file path"});
  }
  ...
  if (p.is_relative()) {
    if (directory.empty()) {
      return std::unexpected(ExtractError{
          .message = std::format("relative file path '{}' requires a non-empty "
                                 "'directory' in compile_commands.json",
                                 path),
      });
    }
...
auto filter_root_path(const config::TaskConfig &config)
    -> std::filesystem::path {
  namespace fs = std::filesystem;
...
template <typename T>
void append_unique_range(std::vector<T> &values,
...
auto rebuild_lookup_maps(ProjectModel &model) -> void {
  ...
  for (auto &[symbol_id, sym] : model.symbols) {
    if (!sym.qualified_name.empty()) {
      model.symbol_ids_by_qualified_name[sym.qualified_name].push_back(
          symbol_id);
    }
  }

  for (auto &[qualified_name, symbol_ids] :
       model.symbol_ids_by_qualified_name) {
    std::sort(symbol_ids.begin(), symbol_ids.end());
    symbol_ids.erase(std::unique(symbol_ids.begin(), symbol_ids.end()),
                     symbol_ids.end());
    if (symbol_ids.size() > 1) {
      logging::info("qualified name '{}' has {} overload candidates",
                    qualified_name, symbol_ids.size());
    }
  }
  ...
  for (auto &[module_name, sources] : model.module_name_to_sources) {
    ...
    if (sources.size() > 1) {
      logging::warn("duplicate module name '{}' from '{}' and '{}'",
                    module_name, sources.front(), sources[1]);
    }
  }
}

auto ensure_namespace_hierarchy(ProjectModel &model,
...
auto find_enclosing_namespace(const ProjectModel &model, const SymbolInfo &sym)
    -> std::string {
...
auto merge_symbol_info_impl(SymbolInfo &current, T &&incoming) -> void {
  ...
  const bool prefer_incoming_definition =
      incoming_ref.definition_location.has_value() &&
      !current.definition_location.has_value();
  ...
  if (current.enclosing_namespace.empty() &&
      !incoming_ref.enclosing_namespace.empty()) {
    current.enclosing_namespace = incoming_ref.enclosing_namespace;
  }
  if (!current.declaration_location.is_known() &&
      incoming_ref.declaration_location.is_known()) {
    current.declaration_location = incoming_ref.declaration_location;
  }
  if ((!current.definition_location.has_value() &&
       incoming_ref.definition_location.has_value()) ||
      (prefer_incoming_definition &&
       incoming_ref.definition_location.has_value())) {
  ...
  } else if (current.source_snippet.empty() &&
             !incoming_ref.source_snippet.empty()) {
    current.source_snippet = incoming_ref.source_snippet;
  }
  ...
  if (current.lexical_parent_name.empty() &&
      !incoming_ref.lexical_parent_name.empty()) {
  ...
  } else if (current.template_params.empty() &&
             !incoming_ref.template_params.empty()) {
    current.template_params = incoming_ref.template_params;
  }
...
auto merge_symbol_info(SymbolInfo &current, const SymbolInfo &incoming)
    -> void {
  merge_symbol_info_impl(current, incoming);
}

auto rebuild_model_indexes(const config::TaskConfig &config,
                           ProjectModel &model) -> void {
  namespace fs = std::filesystem;
...
auto build_module_info(ProjectModel &model, const ScanCache &scan_cache)
  ...
  grouped_modules.reserve(scan_cache.size());

  for (const auto &[cache_key, scan_result] : scan_cache) {
    ...
    if (!key_parts.has_value()) {
      return std::unexpected(ExtractError{
          .message = std::format("invalid scan cache key '{}': {}", cache_key,
                                 key_parts.error().message),
      });
    }

    auto source_file =
        fs::path(key_parts->path).lexically_normal().generic_string();

    auto [group_it, inserted] = grouped_modules.try_emplace(
        source_file, GroupedModuleInfo{
    ...
    if (!inserted && group.name != scan_result.module_name) {
      return std::unexpected(ExtractError{
          .message = std::format(
              "conflicting module names for source file '{}': '{}' vs '{}'",
              source_file, group.name, scan_result.module_name),
      });
    }

    if (!inserted && group.is_interface != scan_result.is_interface_unit) {
      return std::unexpected(ExtractError{
          .message = std::format("conflicting module interface flag for source "
                                 "file '{}': {} vs {}",
                                 source_file, group.is_interface,
...
auto extract_project(const config::TaskConfig &config)
  ...
  logging::info("loaded {} compile entries ({}ms)", db.entries.size(),
                dt_load.count());
  ...
  for (auto &entry : db.entries) {
    ...
    auto abs_path =
        resolve_path_under_directory(entry_copy.file, entry_copy.directory);
  ...
  for (auto &entry : filtered_db.entries) {
    ...
    if (auto cache_it = cache_records.find(entry.cache_key);
        cache_it != cache_records.end()) {
  ...
  logging::info("dependency graph: {} files, {} edges ({}ms)",
                dep_graph.files.size(), dep_graph.edges.size(),
                dt_graph.count());
  ...
  for (std::size_t idx = 0; idx < filtered_db.entries.size(); ++idx) {
    ...
    if (cache_state_it == cache_evaluations.end()) {
      return std::unexpected(ExtractError{
          .message = std::format("missing extract cache evaluation for {}",
                                 normalized)});
    }
    ...
    if (cache_state_it->second.ast_valid &&
        cache_record_it != cache_records.end()) {
      ...
      logging::info("  ast cache: {} symbols, {} relations ({}ms)",
                    ast_view->symbols.size(), ast_view->relations.size(),
                    dt_ast.count());
    } else {
      auto ast_result =
          extract_symbols(entry, *config.extract.max_snippet_bytes);
      ...
      ast_view = &ast_data;
      auto ast_deps_result =
          cache::capture_dependency_snapshot(ast_data.dependencies);
      ...
      logging::info("  ast: {} symbols, {} relations ({}ms)",
                    ast_data.symbols.size(), ast_data.relations.size(),
                    dt_ast.count());
    }
    ...
    if (cache_it == scan_cache.end()) {
      ...
      auto [rescanned_it, _] =
          scan_cache.insert_or_assign(cache_key, std::move(*scan_result));
    ...
    for (const auto &sym : ast_view->symbols) {
      ...
      auto owner_path_result = resolve_path_under_directory(
          sym.declaration_location.file, entry.directory);
      ...
      if (normalized_sym.definition_location.has_value() &&
        ...
        auto definition_path_result = resolve_path_under_directory(
            normalized_sym.definition_location->file, entry.directory);
      ...
      if (sym_it == model.symbols.end()) {
        auto [inserted_it, _] =
            model.symbols.emplace(symbol_id, std::move(normalized_sym));
        sym_it = inserted_it;
        ++symbols_kept;
      } else {
    ...
    logging::info("  kept: {} symbols, {} includes ({}ms)", symbols_kept,
                  includes_kept, dt_file.count());
  }
  ...
  auto t3 = Clock::now();
  logging::info("building reverse edges for {} symbols...",
                model.symbols.size());
  ...
  if (auto module_result = build_module_info(model, scan_cache);
      !module_result.has_value()) {
  ...
  if (model.uses_modules) {
    ...
    for (auto &[source_file, mod] : model.modules) {
      logging::info("  module '{}' (interface={}) from {}", mod.name,
                    mod.is_interface, source_file);
    }
  }

  auto save_cache_result =
      cache::save_extract_cache(config.workspace_root, cache_records);
  if (!save_cache_result.has_value()) {
    logging::warn(
        "failed to save extract cache; continuing without persisted cache: {}",
  ...
  logging::info(
      "project model: {} symbols, {} files, {} namespaces ({} skipped)",
      model.symbols.size(), model.files.size(), model.namespaces.size(),
      skipped);
  logging::info("relations: {} call edges, {} reference edges", total_calls,
                total_refs);
  logging::info("timing: load={}ms  graph={}ms  extract={}ms  total={}ms",
                dt_load.count(), dt_graph.count(), dt_extract.count(),
                dt_total.count());
