...
auto scan_file(const CompileEntry &entry)
...
auto scan_module_decl(std::string_view file_content, ScanResult &result)
    -> void;
...
auto append_unique_import(ScanResult &result, std::string import_name) -> void {
  if (std::ranges::find(result.module_imports, import_name) !=
...
auto normalize_partition_import(std::string_view current_module_name,
  ...
  if (import_name.starts_with(':') && !current_module_name.empty()) {
    ...
    if (auto colon_pos = current_module_name.find(':');
...
public:
  ...
  void InclusionDirective(
      ...
      clang::OptionalFileEntryRef file, llvm::StringRef /*search_path*/,
...
auto scan_module_decl(std::string_view file_content, ScanResult &result)
  ...
  llvm::SmallVector<clang::dependency_directives_scan::Directive, 16>
      directives;

  if (clang::scanSourceForDependencyDirectives(file_content, tokens,
                                               directives)) {
  ...
  for (auto &dir : directives) {
    if (dir.Kind == dds::cxx_export_module_decl ||
        dir.Kind == dds::cxx_module_decl) {
      ...
      if (!import_name.empty()) {
        append_unique_import(
            result, normalize_partition_import(result.module_name,
...
auto scan_file(const CompileEntry &entry)
  ...
  if (entry.arguments.empty()) {
    return std::unexpected(
        ScanError{.message = std::format("empty argument list for file: {}",
                                         entry.file)});
  }
  ...
  if (!instance) {
    return std::unexpected(ScanError{
        .message = std::format(
            "failed to create compiler instance for file: {}", entry.file)});
  }
  ...
  if (!action->BeginSourceFile(*instance,
    ...
    return std::unexpected(ScanError{
        .message = std::format("failed to begin preprocessing for file: {}",
                               entry.file)});
  }
  ...
  if (auto error = action->Execute()) {
    ...
    return std::unexpected(ScanError{
        .message =
            std::format("preprocessor scan failed for file: {}", entry.file)});
  }
