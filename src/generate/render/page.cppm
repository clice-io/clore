...
using PageLinkTarget = LinkTarget;
using PageSymbolDocView = SymbolDocView;

auto find_doc_index_path_page(const PageDocLayout& layout, std::string_view qualified_name)
    -> const std::string* {
    auto it = layout.index_paths.find(std::string(qualified_name));
    return it != layout.index_paths.end() ? &it->second : nullptr;
}

auto find_doc_detail_path_page(const PageDocLayout& layout, std::string_view qualified_name)
    -> const std::string* {
    auto it = layout.detail_paths.find(std::string(qualified_name));
    return it != layout.detail_paths.end() ? &it->second : nullptr;
}

auto add_symbol_doc_links_page(std::vector<MarkdownNode>& nodes,
                               std::string_view current_page_path,
                               const PageDocLayout& layout,
                               const extract::SymbolInfo& sym,
                               PageSymbolDocView view) -> void {
    std::vector<PageLinkTarget> targets;
    if(auto* index_path = find_doc_index_path_page(layout, sym.qualified_name);
       index_path != nullptr && *index_path != current_page_path) {
        targets.push_back(make_link_target(current_page_path, "Overview", *index_path));
    }
    if(auto* detail_path = find_doc_detail_path_page(layout, sym.qualified_name);
...
auto build_namespace_page_root(const PagePlan& plan,
    ...
    append_standard_symbol_sections(
        ...
        [&](std::vector<MarkdownNode>& nodes, const extract::SymbolInfo& sym) {
            add_symbol_doc_links_page(nodes,
                                      plan.relative_path,
                                      layout,
                                      sym,
    ...
    if(auto* module = extract::find_module_by_name(model, plan.owner_keys.front())) {
        root->children.push_back(MarkdownNode{build_list_section("Imports", 2, [&]() {
            ...
            for(const auto& imported: module->imports) {
                ListItem item;
                if(auto* target_path = links.resolve_module(imported)) {
                    item.fragments.push_back(
                        make_link(imported,
                                  make_relative_link_target(plan.relative_path, *target_path),
                                  true));
