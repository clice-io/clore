...
TEST_SUITE(scan){TEST_CASE(topological_order_simple){DependencyGraph graph;
...
TEST_CASE(build_dependency_graph_normalizes_relative_includes_like_entries) {
  ...
  auto root = (fs::temp_directory_path() / "clore_scan_normalization")
                  .lexically_normal();
  auto source = canonical_test_path(root / "src" / "main.cpp");
  ...
  ScanCache initial_cache;
  initial_cache.emplace(source + "\t1",
                        ScanResult{
...
TEST_CASE(build_dependency_graph_canonicalizes_symlinked_entries_and_includes) {
  ...
  auto root = (fs::temp_directory_path() / "clore_scan_symlink_normalization")
                  .lexically_normal();
  std::error_code ec;
  fs::remove_all(root, ec);
  fs::create_directories(root / "src");
  ...
  auto header_alias =
      (root / "alias" / "header.hpp").lexically_normal().generic_string();
  ...
  initial_cache.emplace(header_alias + "\t2", ScanResult{});
  ...
  ASSERT_EQ(result->graph.edges.size(), 1u);
  EXPECT_EQ(result->graph.edges[0].from, source);
  EXPECT_EQ(result->graph.edges[0].to, header_real);
  EXPECT_TRUE(std::ranges::find(result->graph.files, header_real) !=
              result->graph.files.end());
