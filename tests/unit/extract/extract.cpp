...
TEST_SUITE(extract_integration){
    TEST_CASE(full_pipeline){namespace fs = std::filesystem;
...
task_config.compile_commands_path =
    (temp_dir / "compile_commands.json").string();
...
auto math_cpp =
    (temp_dir / "src" / "math.cpp").lexically_normal().generic_string();
...
int add(int lhs, int rhs) {
  ...
  auto cache_path =
      root / ".clice" / "cache" / "clore" / "extract" / "cache.json";
...
export int detail() {
  ...
  auto math_path =
      (root / "src" / "math.cppm").lexically_normal().generic_string();
  auto detail_path =
      (root / "src" / "math.detail.cppm").lexically_normal().generic_string();
...
TEST_CASE(merges_cached_module_imports_for_duplicate_compile_entries) {
  ...
  auto ticks = std::chrono::steady_clock::now().time_since_epoch().count();
  auto root = fs::temp_directory_path() /
              std::format("clore_module_import_merge_test_{}", ticks);
  fs::create_directories(root / "src");
...
export int run() {
  ...
  auto source = source_file.lexically_normal().generic_string();
  ...
  auto save_cache =
      extract::cache::save_extract_cache(root.string(), cache_records);
...
TEST_CASE(rejects_conflicting_cached_module_metadata_for_same_source_file) {
  ...
  auto ticks = std::chrono::steady_clock::now().time_since_epoch().count();
  auto root = fs::temp_directory_path() /
              std::format("clore_module_conflict_test_{}", ticks);
  fs::create_directories(root / "src");
...
export int run() {
  ...
  std::unordered_map<std::string, extract::cache::CacheRecord> cache_records;
  cache_records.emplace(
      extract::cache::build_cache_key(source, alpha_signature),
      extract::cache::CacheRecord{
  ...
  EXPECT_NE(
      result.error().message.find("conflicting module names for source file"),
      std::string::npos);
...
auto shared_header =
    (root / "src" / "shared.h").lexically_normal().generic_string();
...
EXPECT_EQ(std::count(model.files.at(shared_header).symbols.begin(),
                     model.files.at(shared_header).symbols.end(), shared_id),
          1);
EXPECT_EQ(std::count(model.files.at(shared_header).symbols.begin(),
                     model.files.at(shared_header).symbols.end(), namespace_id),
          1);
EXPECT_EQ(std::count(model.files.at(load_cpp).symbols.begin(),
                     model.files.at(load_cpp).symbols.end(), shared_id),
          0);
EXPECT_EQ(std::count(model.symbols.at(namespace_id).children.begin(),
                     model.symbols.at(namespace_id).children.end(), shared_id),
          1);

auto parent_namespace_it =
    std::ranges::find_if(model.symbols, [](const auto &item) {
      return item.second.kind == extract::SymbolKind::Namespace &&
...
EXPECT_EQ(std::count(parent_namespace_it->second.children.begin(),
                     parent_namespace_it->second.children.end(), namespace_id),
          1);
...
TEST_SUITE(scan_error_propagation){
    ...
    TEST_CASE(empty_args_returns_error){extract::CompileEntry entry;
entry.file = "test.cpp";
entry.directory = "/tmp";
