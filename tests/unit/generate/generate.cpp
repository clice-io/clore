...
auto make_basic_model(const fs::path& project_root) -> extract::ProjectModel {
    ...
    auto symbol = make_function_symbol(1, "add", "math::add", "int add(int lhs, int rhs)",
                                       file_path, "math");
    auto symbol_id = symbol.id;
...
auto make_prompt_outputs(std::initializer_list<PromptRequest> requests,
                         std::string_view text) -> std::unordered_map<std::string, std::string> {
...
TEST_SUITE(generate) {
    ...
    TEST_CASE(build_page_plan_set_creates_index_and_file_pages_by_default) {
        ...
        EXPECT_TRUE(std::ranges::any_of(index_plan->prompt_requests,
        ...
        EXPECT_TRUE(std::ranges::any_of(file_plan->prompt_requests,
                                        [](const PromptRequest& request) {
    ...
    TEST_CASE(build_page_plan_set_uses_modules_to_emit_modules_not_files) {
        ...
        auto util_symbol = make_function_symbol(10, "helper", "demo::util::helper",
                                                "int helper()", util_file, "demo::util");
    ...
    TEST_CASE(build_page_plan_set_keeps_nested_types_on_their_owner_page) {
        ...
        EXPECT_TRUE(std::ranges::any_of(namespace_plan->prompt_requests,
                                        [](const PromptRequest& request) {
                                            return request.kind == PromptKind::TypeDeclarationSummary &&
                                                   request.target_key == "demo::Outer";
                                        }));
        EXPECT_FALSE(std::ranges::any_of(namespace_plan->prompt_requests,
                                         [](const PromptRequest& request) {
                                             return request.kind == PromptKind::TypeDeclarationSummary &&
    ...
    TEST_CASE(build_page_plan_set_orders_independent_modules_lexicographically) {
        ...
        ASSERT_TRUE(result.has_value());
        auto module_ids = result->generation_order |
                          std::views::filter([](const std::string& id) {
    ...
    TEST_CASE(render_page_markdown_relativizes_workflow_links_in_index) {
        ...
        auto links = build_link_resolver(PagePlanSet{.plans = {index_plan, workflow_plan}});
        auto outputs = make_prompt_outputs(
            {PromptRequest{.kind = PromptKind::IndexOverview},
    ...
    TEST_CASE(render_page_markdown_relativizes_file_links_in_index) {
        ...
        auto links = build_link_resolver(PagePlanSet{.plans = {index_plan, file_plan}});
        auto outputs = make_prompt_outputs(
            {PromptRequest{.kind = PromptKind::IndexOverview},
             PromptRequest{.kind = PromptKind::IndexReadingGuide}},
            "Overview");
    ...
    TEST_CASE(render_page_markdown_links_type_declarations_and_implementations_bidirectionally) {
        ...
        auto links = build_link_resolver(PagePlanSet{.plans = {namespace_plan, module_plan}});
    ...
    TEST_CASE(render_page_markdown_orders_type_members_without_code_blocks) {
        ...
        auto markdown = render_page_markdown(namespace_plan, config, model, outputs,
                                             build_link_resolver(PagePlanSet{.plans = {namespace_plan}}));

        ASSERT_TRUE(markdown.has_value());
        EXPECT_EQ(markdown->find("struct Widget {}"), std::string::npos);
        EXPECT_EQ(markdown->find("void run() { return 0; }"), std::string::npos);

        auto member_types = markdown->find("#### Member Types");
        auto member_variables = markdown->find("#### Member Variables");
        auto member_functions = markdown->find("#### Member Functions");
    ...
    TEST_CASE(namespace_summary_prompt_uses_namespace_subject) {
        ...
        EXPECT_TRUE(prompt->starts_with("Write a summary for the namespace `demo::config`."));
    }
