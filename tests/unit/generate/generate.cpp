...
auto make_function_symbol(std::uint64_t id, std::string_view name,
...
auto add_symbol(extract::ProjectModel &model, extract::SymbolInfo symbol)
    -> void {
...
auto add_namespace(extract::ProjectModel &model, std::string name,
...
auto add_module(extract::ProjectModel &model, extract::ModuleUnit module)
    -> void {
...
auto make_basic_model(const fs::path &project_root) -> extract::ProjectModel {
  ...
  auto symbol = make_function_symbol(
      1, "add", "math::add", "int add(int lhs, int rhs)", file_path, "math");
  auto symbol_id = symbol.id;
...
auto make_prompt_outputs(std::initializer_list<PromptRequest> requests,
...
TEST_CASE(generate_dry_run_emits_llms_txt) {
  ...
  EXPECT_NE(llms_page->content.find("[API Reference](index.md)"),
            std::string::npos);
  EXPECT_NE(llms_page->content.find("[math](namespaces/math/index.md)"),
            std::string::npos);
  EXPECT_NE(llms_page->content.find("[src/math.cpp](files/src/math.md)"),
            std::string::npos);
}
...
TEST_CASE(build_page_plan_set_creates_index_and_file_pages_by_default) {
  ...
  auto index_plan =
      std::ranges::find_if(result->plans, [](const PagePlan &plan) {
  ...
  EXPECT_TRUE(std::ranges::any_of(
      index_plan->prompt_requests, [](const PromptRequest &request) {
        return request.kind == PromptKind::IndexOverview;
      }));
  EXPECT_TRUE(std::ranges::any_of(
      file_plan->prompt_requests, [](const PromptRequest &request) {
...
TEST_CASE(build_page_plan_set_uses_modules_to_emit_modules_only) {
  ...
  auto util_symbol =
      make_function_symbol(10, "helper", "demo::util::helper", "int helper()",
                           util_file, "demo::util");
  auto api_symbol = make_function_symbol(11, "add", "demo::math::add",
                                         "int add(int lhs, int rhs)", main_file,
                                         "demo::math");
  ...
  auto pos_util =
      std::ranges::find(result->generation_order, "module:demo.util");
  auto pos_math =
      std::ranges::find(result->generation_order, "module:demo.math");
...
TEST_CASE(build_page_plan_set_keeps_parent_only_namespace_pages) {
  ...
  auto type_symbol =
      make_type_symbol(200, "Options", "demo::config::Options", file,
                       "Configuration options.", "demo::config");
  ...
  EXPECT_TRUE(
      std::ranges::find(root_plan->linked_pages, "namespace:demo::config") !=
...
TEST_CASE(build_page_plan_set_keeps_nested_types_on_their_owner_page) {
  ...
  auto outer = make_type_symbol(300, "Outer", "demo::Outer", file,
                                "Outer type.", "demo");
  auto inner = make_type_symbol(301, "Inner", "demo::Outer::Inner", file,
                                "Nested helper.", "demo");
  ...
  add_namespace(
      model, "demo",
      {extract::SymbolID{.hash = 300}, extract::SymbolID{.hash = 301}});
  ...
  EXPECT_TRUE(std::ranges::any_of(
      namespace_plan->prompt_requests, [](const PromptRequest &request) {
...
TEST_CASE(build_page_plan_set_skips_function_local_types_from_page_entries) {
  ...
  auto exported = make_type_symbol(310, "Client", "demo::Client", file,
                                   "Client type.", "demo");
  auto local = make_type_symbol(311, "ActiveRunReset", "ActiveRunReset", file,
                                "Local helper.", "demo");
  ...
  add_namespace(
      model, "demo",
      {extract::SymbolID{.hash = 310}, extract::SymbolID{.hash = 311}});
...
TEST_CASE(build_page_plan_set_orders_independent_modules_lexicographically) {
  ...
  ASSERT_TRUE(result.has_value());
  auto module_ids =
      result->generation_order | std::views::filter([](const std::string &id) {
...
TEST_CASE(render_page_markdown_relativizes_file_links_in_index) {
  ...
  auto outputs = make_prompt_outputs(
      {PromptRequest{.kind = PromptKind::IndexOverview}}, "Overview");
...
TEST_CASE(
    render_page_markdown_links_type_declarations_and_implementations_bidirectionally) {
  ...
  auto widget = make_type_symbol(400, "Widget", "demo::Widget", file,
                                 "Widget type.", "demo");
  ...
  auto outputs = std::unordered_map<std::string, std::string>{
      {prompt_request_key(PromptRequest{.kind = PromptKind::NamespaceSummary}),
       "Summary"},
      {prompt_request_key(
           PromptRequest{.kind = PromptKind::TypeDeclarationSummary,
                         .target_key = widget_key}),
       "Declaration"},
      {prompt_request_key(PromptRequest{.kind = PromptKind::ModuleSummary}),
       "Summary"},
      {prompt_request_key(
           PromptRequest{.kind = PromptKind::ModuleArchitecture}),
       "Architecture"},
  ...
  PagePlanSet plan_set{.plans = {namespace_plan, module_plan}};
  auto links = build_link_resolver(plan_set);

  auto namespace_markdown =
      render_page_markdown(namespace_plan, config, model, outputs, links);
  auto module_markdown =
  ...
  EXPECT_NE(namespace_markdown->find(
                "[Module demo.math](../../modules/demo/math/index.md)"),
            std::string::npos);
  EXPECT_NE(module_markdown->find(
                "[Namespace demo](../../../namespaces/demo/index.md)"),
            std::string::npos);
...
TEST_CASE(
    render_page_markdown_emits_file_dependency_diagram_when_graph_is_nontrivial) {
  ...
  auto state = make_type_symbol(450, "State", "demo::State", file,
                                "State type.", "demo");
  auto run =
      make_function_symbol(451, "run", "demo::run", "void run()", file, "demo");
...
TEST_CASE(render_page_markdown_omits_file_dependency_diagram_for_tiny_graph) {
  ...
  auto run =
      make_function_symbol(452, "run", "demo::run", "void run()", file, "demo");
  add_symbol(model, std::move(run));
...
TEST_CASE(
    render_page_bundle_places_complex_type_members_in_nested_doc_page_without_code_blocks) {
  ...
  auto widget = make_type_symbol(500, "Widget", "demo::Widget", file,
                                 "Widget type.", "demo");
  auto widget_key = make_symbol_target_key(widget);
  auto nested = make_type_symbol(501, "Options", "demo::Widget::Options", file,
                                 "Nested options.", "demo");
  ...
  state.declaration_location =
      extract::SourceLocation{.file = file, .line = 4, .column = 1};

  auto run = make_function_symbol(503, "run", "demo::Widget::run", "void run()",
                                  file, "demo");
  ...
  add_namespace(
      model, "demo",
      {extract::SymbolID{.hash = 500}, extract::SymbolID{.hash = 501}});
  ...
  auto links = build_link_resolver(plan_set);
  auto bundle =
      render_page_bundle(namespace_plan, config, model, outputs, links);

  ASSERT_TRUE(bundle.has_value());
  auto root_page =
      std::ranges::find_if(*bundle, [&](const GeneratedPage &page) {
  ...
  EXPECT_EQ(detail_page->content.find("void run() { return 0; }"),
            std::string::npos);
...
TEST_CASE(
    function_declaration_evidence_uses_signature_aware_related_summaries) {
  ...
  auto caller_int = make_function_symbol(700, "caller", "demo::caller",
                                         "int caller(int value)", file, "demo");
  auto caller_int_id = caller_int.id;
  auto caller_int_key = make_symbol_target_key(caller_int);

  auto caller_double =
      make_function_symbol(701, "caller", "demo::caller",
                           "double caller(double value)", file, "demo");
  auto caller_double_id = caller_double.id;
  auto caller_double_key = make_symbol_target_key(caller_double);

  auto target = make_function_symbol(702, "target", "demo::target",
                                     "void target()", file, "demo");
  ...
  add_namespace(
      model, "demo",
      {caller_int_id, caller_double_id, extract::SymbolID{.hash = 702}});

  PageSummaryCache summaries{
      {caller_int_key, "int overload summary"},
      {caller_double_key, "double overload summary"},
  };

  auto *symbol = extract::find_symbol(model, "demo::target", "void target()");
  ASSERT_TRUE(symbol != nullptr);

  auto evidence = build_evidence_for_function_declaration_summary(
      *symbol, model, config.evidence_rules, summaries, config.project_root);

  ASSERT_EQ(evidence.related_page_summaries.size(), 2u);
  EXPECT_TRUE(std::ranges::find(evidence.related_page_summaries,
                                "int overload summary") !=
              evidence.related_page_summaries.end());
  EXPECT_TRUE(std::ranges::find(evidence.related_page_summaries,
                                "double overload summary") !=
              evidence.related_page_summaries.end());
}
...
TEST_CASE(type_declaration_evidence_uses_signature_aware_related_summaries) {
  ...
  auto base =
      make_type_symbol(710, "Base", "demo::Base", file, "Base type.", "demo");
  auto base_id = base.id;
  auto base_key = make_symbol_target_key(base);

  auto derived = make_type_symbol(711, "Derived", "demo::Derived", file,
                                  "Derived type.", "demo");
  ...
  PageSummaryCache summaries{
      {base_key, "base type summary"},
  };

  auto *symbol = extract::find_symbol(model, "demo::Derived", "struct Derived");
  ASSERT_TRUE(symbol != nullptr);

  auto evidence = build_evidence_for_type_declaration_summary(
      *symbol, model, config.evidence_rules, summaries, config.project_root);
...
TEST_CASE(namespace_summary_prompt_uses_namespace_subject) {
  ...
  for (std::uint64_t i = 0; i < 3; ++i) {
    ...
    auto symbol = make_type_symbol(100 + i, name, qualified_name, file,
                                   "Type documentation.", "demo::config");
  ...
  auto evidence = build_evidence_for_namespace_summary(
      model.namespaces.at(ns.name), model, config.evidence_rules, summaries,
      config.project_root);
  ...
  EXPECT_NE(prompt->find("Write a summary for the namespace `demo::config`."),
            std::string::npos);
}

TEST_CASE(
    function_declaration_summary_deduplicates_and_caps_reverse_usage_context) {
  ...
  auto target = make_function_symbol(900, "target", "demo::target",
                                     "void target()", file, "demo");
  auto caller_a = make_function_symbol(901, "callerA", "demo::callerA",
                                       "void callerA()", file, "demo");
  auto caller_b = make_function_symbol(902, "callerB", "demo::callerB",
                                       "void callerB()", file, "demo");
  auto ref_only = make_function_symbol(903, "refOnly", "demo::refOnly",
                                       "void refOnly()", file, "demo");
  ...
  add_namespace(model, "demo",
                {target.id, caller_a.id, caller_b.id, ref_only.id});

  PageSummaryCache summaries;
  auto evidence = build_evidence_for_function_declaration_summary(
      target, model, config.evidence_rules, summaries, config.project_root);
...
TEST_CASE(
    type_declaration_summary_deduplicates_and_caps_reverse_usage_context) {
  ...
  auto target = make_type_symbol(910, "Widget", "demo::Widget", file,
                                 "Widget doc.", "demo");
  auto use_a = make_function_symbol(911, "useA", "demo::useA", "void useA()",
                                    file, "demo");
  auto use_b = make_function_symbol(912, "useB", "demo::useB", "void useB()",
                                    file, "demo");
  auto derived = make_type_symbol(913, "Derived", "demo::Derived", file,
                                  "Derived doc.", "demo");
...
TEST_CASE(render_page_markdown_strips_prompt_code_fence_markers) {
  ...
  auto file = (temp.path / "src" / "math.cppm").generic_string();
  auto sym =
      make_function_symbol(700, "run", "demo::run", "void run()", file, "demo");
  ...
  auto outputs = std::unordered_map<std::string, std::string>{
      ...
      {prompt_request_key(
           PromptRequest{.kind = PromptKind::FunctionImplementationSummary,
                         .target_key = run_key}),
       "Algorithm note with inline marker "
       "```helper```.\n\n```cpp\nstep();\n```"},
  };
...
TEST_CASE(
    render_page_bundle_emits_nested_symbol_docs_for_complex_module_types) {
  ...
  auto widget = make_type_symbol(800, "Widget", "demo::Widget", file,
                                 "Widget type.", "demo");
  auto options = make_type_symbol(801, "Options", "demo::Widget::Options", file,
                                  "Nested options.", "demo");
  ...
  auto run = make_function_symbol(803, "run", "demo::Widget::run", "void run()",
                                  file, "demo");
  run.kind = extract::SymbolKind::Method;
  run.parent = widget.id;
  run.lexical_parent_name = "demo::Widget";
  ...
  EXPECT_NE(root_page->content.find("[Overview](types/widget.md)"),
            std::string::npos);
  EXPECT_NE(detail_page->content.find("## Structure Diagram"),
            std::string::npos);
...
TEST_CASE(
    render_markdown_clamps_heading_levels_and_prefixes_multiline_blockquotes) {
  MarkdownDocument document;
  auto section =
      make_section(SemanticKind::Section, "demo", "Deep Section", 9, false);
